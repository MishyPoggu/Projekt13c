...
const addArcadeMachine = async (req, res) => {
    ...
    try {
        if (isAdmin(req)) {
            ...
            const existingName = await ArcadeMachines.findOne({where: { name }, transaction});
            if (existingName) {
...
const updateArcadeMachine = async (req, res) => {
    ...
    try {
        if (isAdmin(req)) {
            ...
            const existingArcadeMachine = await ArcadeMachines.findByPk(id, { transaction });
...
const addArcadeMachines = async (req, res) => {
    ...
    try {
        if (isAdmin(req)) {
            ...
            if (!arcadeMachines || !Array.isArray(arcadeMachines) || arcadeMachines.length === 0) {
                return res.status(400).json({
                    status: 400,
            ...
            for (const arcadeData of arcadeMachines) {
                ...
                const existingArcade = await ArcadeMachines.findOne({ where: { name }, transaction });
                if (existingArcade) {
                    await transaction.rollback();
                    return res.status(409).json({
                        status: 409,
