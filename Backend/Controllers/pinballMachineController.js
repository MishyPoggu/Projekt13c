...
const addPinballMachine = async (req, res) => {
    ...
    try {
        if (isAdmin(req)) {
            ...
            const existingName = await PinballMachines.findOne({where: { name }, transaction});
            if (existingName) {
...
const updatePinballMachine = async (req, res) => {
    ...
    try {
        if (isAdmin(req)) {
            ...
            const existingPinballMachine = await PinballMachines.findByPk(id, { transaction });
...
const addPinballMachines = async (req, res) => {
    ...
    try {
        if (isAdmin(req)) {
            ...
            if (!pinballMachines || !Array.isArray(pinballMachines) || pinballMachines.length === 0) {
                return res.status(400).json({
                    status: 400,
            ...
            for (const pinballData of pinballMachines) {
                ...
                console.log(`Checking for existing pinball machine with name: ${name}...`);
                const existingPinballMachine = await PinballMachines.findOne({ where: { name }, transaction });
                if (existingPinballMachine) {
                    await transaction.rollback();
                    return res.status(409).json({
                        status: 409,
