...
export class UserprofileComponent implements OnInit {
  ...
  initForm() {
    this.profilForm = new FormGroup({
      ...
      profilePic: new FormControl(''), // Nem kötelező mostantól
  ...
  updateValidators() {
    if (this.isCompanyLogin) {
      ...
      this.profilForm.get('profilePic')?.clearValidators();  // Nem kötelező a profilkép
      ...
      this.profilForm.get('profilePic')?.clearValidators();  // Profilkép nem kötelező
    }
  ...
  saveProfile() {
    if (this.profilForm.valid) {
      const saveObservable = this.isCompanyLogin
      ...
      saveObservable.subscribe(
        (response) => {
          ...
          // Frissítem a formot a mentett adatokkal
          this.loadUserProfile(); // Profil betöltése újra, hogy friss adatokat kapjunk
        },
        (error) => {
      ...
      alert('Kérlek, ellenőrizd az űrlap mezőit!');
    }
  }
}